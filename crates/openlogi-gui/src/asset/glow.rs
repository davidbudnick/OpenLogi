...
pub(crate) fn ensure_glow_png(depot: &str, hex: &str) -> Option<PathBuf> {
    ...
    let color = Rgba([(rgb >> 16) as u8, (rgb >> 8) as u8, rgb as u8, 255]);
    ...
    // half-written PNG — gpui caches an image load *failure* permanently, which
    // would leave the card blank for the rest of the session.
...
fn paint_mask(mask: &GlowMask, color: Rgba<u8>) -> Option<RgbaImage> {
    ...
    if total == 0 || mask.runs.iter().map(|&r| u64::from(r)).sum::<u64>() != total {
...
/// Remove other `glow-*.png` colour variants in `dir` so the cache doesn't grow
/// unbounded as colours are tried.
fn prune_stale(dir: &Path, keep: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        let is_variant = path
            .file_name()
            .and_then(|n| n.to_str())
            .is_some_and(|n| n.starts_with("glow-") && n.ends_with(".png"));
        if is_variant && path != keep {
