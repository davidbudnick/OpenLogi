...
impl DetailTab {
    ...
    fn tabs_for(record: &DeviceRecord) -> Vec<Self> {
        ...
        // A keyboard never gets the pointer-device tabs even if its HID++
        // reprogrammable-controls probe reports buttons — those are the mouse's
        // remap/DPI sections and read as odd on a keyboard.
        let pointer_device = record.kind != DeviceKind::Keyboard;
        let mut tabs = Vec::new();
        if caps.buttons && pointer_device {
            tabs.push(Self::Buttons);
        }
        if caps.pointer && pointer_device {
    ...
    fn ensure_glow(cx: &mut Context<Self>) {
        let jobs: Vec<GlowJob> = {
            ...
            state
                ...
                .filter(|job| !job.cache.exists())
                .collect()
        };
        for job in jobs {
            let newly = cx
                .update_global::<AppState, _>(|state, _| state.mark_glow_inflight(job.cache.clone()));
            if !newly {
                continue;
            }
            ...
            std::thread::spawn(move || {
                let _ = crate::asset::ensure_glow_png(&depot, &hex);
                let _ = tx.send(());
            });
            cx.spawn(async move |_view, cx| {
                let _ = rx.await;
                cx.update_global::<AppState, _>(|state, cx| {
                    state.clear_glow_inflight(&cache);
                    cx.refresh_windows();
                });
            })
            .detach();
...
fn glow_job(state: &AppState, record: &DeviceRecord) -> Option<GlowJob> {
    ...
    let lighting = state.lighting_for(&record.config_key).filter(|l| l.enabled)?;
    let asset = record.asset.as_ref()?;
    asset.hero_image_path.as_ref()?;
    Some(GlowJob {
        ...
        .child(
            div()
                ...
                .when_some(glow, |this, path| {
                    this.child(img(path).absolute().top_0().left_0().size_full().opacity(0.6))
                }),
        )
        .child(
            v_flex()
